...
local LIB_BINDS_FILE = "lib_binds_save.json"

local function _loadLibBinds()
	if not (readfile and writefile) then return {} end
	local ok, data = pcall(readfile, LIB_BINDS_FILE)
	if not ok or not data or data:gsub("%s", "") == "" then return {} end
	local ok2, decoded = pcall(HttpService.JSONDecode, HttpService, data)
	if not ok2 then return {} end
	return decoded
end

local function _saveLibBinds(binds)
	if not writefile then return end
	pcall(writefile, LIB_BINDS_FILE, HttpService:JSONEncode(binds))
end

local _libBinds = _loadLibBinds()
-- ========== КОНЕЦ СИСТЕМЫ СОХРАНЕНИЯ БИНД ==========
...
function lib:Window(text, preset, closebind)
	...
	local TabHold = Instance.new("ScrollingFrame")  -- ИЗМЕНЕНО: Frame -> ScrollingFrame
		...
		pcall(function()
			if key.KeyCode == Enum.KeyCode.T then
				if Main.Visible == true then
					game:GetService("TweenService"):Create(Main,TweenInfo.new(.3),{Size = UDim2.new(0,0,0,0)}):Play()
					wait(.3)
					Main.Visible = false
				else
					game:GetService("TweenService"):Create(Main,TweenInfo.new(.3),{Size = UDim2.new(0, 560, 0, 319)}):Play()
					Main.Visible = true
				end
			end
		end)
	...
	-- ========== TABHOLD: ScrollingFrame настройки ==========
	...
	TabHold.AutomaticCanvasSize = Enum.AutomaticSize.Y  -- автоматически растягивает канвас
	TabHold.ScrollBarThickness = 2
	TabHold.ScrollingDirection = Enum.ScrollingDirection.Y
	TabHold.ScrollBarImageColor3 = Color3.fromRGB(80, 80, 80)
	...
	Timer.Position = UDim2.new(0.15, 0, 0.056, 0)
	Timer.Size = UDim2.new(0, 200, 0, 23)
	Timer.Font = Enum.Font.GothamSemibold
	Timer.Text = ""
	Timer.TextColor3 = Color3.fromRGB(130, 130, 130)
	...
	function tabhold:Tab(text)
		...
		function tabcontent:Button(text, callback)
			...
			-- Bind для Button
			local BtnBindLabel = Instance.new("TextButton")
			local BtnBindCorner = Instance.new("UICorner")
			BtnBindLabel.Name = "BtnBindLabel"
			BtnBindLabel.Parent = Button
			BtnBindLabel.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
			BtnBindLabel.Position = UDim2.new(0, 309, 0, 10)
			BtnBindLabel.Size = UDim2.new(0, 47, 0, 22)
			BtnBindLabel.AutoButtonColor = false
			BtnBindLabel.Font = Enum.Font.Gotham
			BtnBindLabel.Text = _libBinds["btn_"..text] and ("["..(_libBinds["btn_"..text]).."]") or "[None]"
			BtnBindLabel.TextColor3 = Color3.fromRGB(160, 160, 160)
			BtnBindLabel.TextSize = 11
			BtnBindCorner.CornerRadius = UDim.new(0, 4)
			BtnBindCorner.Parent = BtnBindLabel

			local btnListening = false
			BtnBindLabel.MouseButton1Click:Connect(function()
				if btnListening then return end
				btnListening = true
				BtnBindLabel.Text = "[...]"
				BtnBindLabel.TextColor3 = Color3.fromRGB(255, 205, 0)
				local conn
				conn = UserInputService.InputBegan:Connect(function(input, gpe)
					...
					if input.UserInputType == Enum.UserInputType.Keyboard then
						...
						if keyName == "Escape" then
							_libBinds["btn_"..text] = nil
							BtnBindLabel.Text = "[None]"
							BtnBindLabel.TextColor3 = Color3.fromRGB(160, 160, 160)
						else
							_libBinds["btn_"..text] = keyName
							BtnBindLabel.Text = "["..keyName.."]"
							BtnBindLabel.TextColor3 = Color3.fromRGB(160, 160, 160)
						end
						_saveLibBinds(_libBinds)
						conn:Disconnect()
						task.defer(function()
							btnListening = false
						end)
			...
			UserInputService.InputBegan:Connect(function(input, gpe)
				if gpe then return end
				if input.UserInputType == Enum.UserInputType.Keyboard then
					if _libBinds["btn_"..text] and input.KeyCode.Name == _libBinds["btn_"..text] and not btnListening then
						pcall(callback)
					end
				end
			end)
		...
		function tabcontent:Toggle(text,default, callback)
			...
			-- Bind для Toggle
			local TogBindLabel = Instance.new("TextButton")
			local TogBindCorner = Instance.new("UICorner")
			TogBindLabel.Name = "TogBindLabel"
			TogBindLabel.Parent = Toggle
			TogBindLabel.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
			TogBindLabel.Position = UDim2.new(0, 258, 0, 10)
			TogBindLabel.Size = UDim2.new(0, 47, 0, 22)
			TogBindLabel.AutoButtonColor = false
			TogBindLabel.Font = Enum.Font.Gotham
			TogBindLabel.Text = _libBinds["tog_"..text] and ("[".._libBinds["tog_"..text].."]") or "[None]"
			TogBindLabel.TextColor3 = Color3.fromRGB(160, 160, 160)
			TogBindLabel.TextSize = 11
			TogBindCorner.CornerRadius = UDim.new(0, 4)
			TogBindCorner.Parent = TogBindLabel

			local togListening = false
			TogBindLabel.MouseButton1Click:Connect(function()
				if togListening then return end
				togListening = true
				TogBindLabel.Text = "[...]"
				TogBindLabel.TextColor3 = Color3.fromRGB(255, 205, 0)
				local conn
				conn = UserInputService.InputBegan:Connect(function(input, gpe)
					if gpe then return end
					if input.UserInputType == Enum.UserInputType.Keyboard then
						local keyName = input.KeyCode.Name
						if keyName == "Escape" then
							_libBinds["tog_"..text] = nil
							TogBindLabel.Text = "[None]"
							TogBindLabel.TextColor3 = Color3.fromRGB(160, 160, 160)
						else
							_libBinds["tog_"..text] = keyName
							TogBindLabel.Text = "["..keyName.."]"
							TogBindLabel.TextColor3 = Color3.fromRGB(160, 160, 160)
						end
						_saveLibBinds(_libBinds)
						conn:Disconnect()
			...
			UserInputService.InputBegan:Connect(function(input, gpe)
				if gpe then return end
				if input.UserInputType == Enum.UserInputType.Keyboard then
					if _libBinds["tog_"..text] and input.KeyCode.Name == _libBinds["tog_"..text] and not togListening then
						if toggled == false then
		...
		function tabcontent:Dropdown(text, list, callback)
			...
			for i, v in next, list do
				...
				if itemcount <= 3 then
					...
					DropItemHolder.Size = UDim2.new(0, 342, 0, 140)
				end
